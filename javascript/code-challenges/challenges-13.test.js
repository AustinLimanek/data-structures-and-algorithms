...
const findHappiness = (arr) => {
  return arr.filter( e => {return /:\)/.test(e)});
};
...
const standardizePhoneNumbers = (arr) => {
  return arr.map(e => [...e].filter(e => parseInt(e)>-1).join(''));
};
...
const onlyOddChars = (str) => {
  return [...str].filter(e => {
    ...
    else if (!parseInt(e) && e.charCodeAt(0) % 2 === 0 && parseInt(e) !== 0) {
      state = true;
    }
    return state;
  }).join('');
...
const allHappy = (arr) => {
  return arr.filter(e => {return /:\)/.test(e)}).length === arr.length;
};
