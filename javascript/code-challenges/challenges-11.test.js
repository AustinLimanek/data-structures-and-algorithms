...
const count = (target, input) => {
  return input.reduce((value, e) => {return [...e, ...value];}, []).reduce((value, e) => { if (e === target){value++;}return value;}, 0);
};
...
const totalSum = (input) => {
  return input.reduce((value, e) => value + e.reduce((value, r) => value + r, 0),0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  let array = input.map(e => e.filter(r => r % 5 === 0 && typeof r !== 'string'));
  return array.map(e => e.map(r => Math.pow(2,r)));
};
...
let findMaleAndFemale = (data) => {
  return data.filter( e => e.gender === 'male' || e.gender === 'female').reduce((value, e) => value + e.name + ' and ', '').slice(0,-5);
};
...
let findShortest = (data) => {
  let num = data.map(e => parseInt(e.height));
  let min = Math.min(...num);
  return data.filter(e => parseInt(e.height) === min)[0].name;
};
/* ------------------------------------------------------------------------------------------------
TESTS
