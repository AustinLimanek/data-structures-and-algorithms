...
const addOne = (arr) => {
  let newArray = [];
  arr.forEach(element => newArray.push(element + 1));
  return newArray;
};
...
const greeting = (word) => {
  let excite = word.toUpperCase() + '!';
  return excite;
};

const speaker = (words, callback) => {
  let message = [];
  words.forEach(element => message.push(callback(element)));
  return message;
};
...
const createList = (availableItems) => {
  ...
  availableItems.forEach(element => {
    if(Object.values(element)[1]){
      shopping.push(Object.values(element)[0]);
    }
  });
  return shopping;
};
