...
const toLastNames = people => {
  return people.map(a => a.firstName + ' ' + a.lastName);
};
...
const addValues = (arr) => {
  return arr.reduce((value, a) => {return value + a;}, 0);
};
...
const addPurchases = (arr) => {
  return arr.reduce((value, a) => {return value + a.purchasePrice;}, 0);
};
...
const countNumberOfElements = (arr) => {
  return arr.reduce(value => {return value + 1;}, 0);
};
...
const returnNames = (arr) => {
  return arr.reduce((Array, a) => {Array.push(a.name); return Array;}, []);
};
...
const reversedString = (str) => {
  let revString = [...str];
  return revString.reduce((string, a) => {return a + string;}, '');
};
...
const countNumberOfChildren = (arr) => {
  return arr.reduce((value, a) => {
    if(a.children){
      return value + a.children.length;
    }
    else{
      return value;
    }
...
const calculateAverage = (arr) => {
  return arr.reduce((value, a) => {return value + a;}, 0)/arr.length;
};
...
const countPrimeNumbers = (arr) => {
  return arr.reduce((value, a) => {if(isPrime(a)){return value + 1;}else{return value;}}, 0);
};
...
const extractStat = (statName, arr) => {
  ...
  return arr.reduce((value, a) => {
    count++;
    if(a.stat.name === statName){
      value = a;
      state = false;
      return value;
    }
    if(count === arr.length && state){
      value = null;
    }
    return value;
  }, {});
...
const extractChildren = (arr) => {
  let aNames = arr.filter(a => a.children && a.name.includes('a'));
  return aNames.reduce((value, a) => [...value, ...a.children], []);
};
