...
const containsAnd = (arr) => {
  return arr.filter(e => e.split('and').length > 1);
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  return arr.filter(e => forbiddenValues.filter(r => r === e).length === 0);
};
...
const getStatName = (arr, minBaseStat) => {
  let statNameArray = [];
  getBaseStatGreaterThan(arr, minBaseStat).map(e => statNameArray.push(e.stat.name));
...
const getCharactersWithoutChildren = (arr) => {
  return arr.filter(e => !(e.children));
};
